...
class DatasetConverter:
    ...
    def _find_medias(self, medias: Union["MediaType", list["MediaType"], None]) -> Optional[list["MediaType"]]:
        ...
        if self.dataset_attr.load_from in ["script", "file"]:
            # 优先使用数据集级别的 media_dir，回退到全局 media_dir
            media_dir = self.dataset_attr.media_dir or self.data_args.media_dir
                ...
                for i in range(len(medias)):
                    ...
                    if os.path.isfile(media_path):
                        ...
                        logger.debug_rank0_once(
                            f"Media {medias[i]} does not exist in dataset media_dir `{media_dir}`. Use original path."
                        )
                ...
                for i in range(len(medias)):
                    for j in range(len(medias[i])):
                        ...
                        if os.path.isfile(media_path):
                            ...
                            logger.debug_rank0_once(
                                f"Media {medias[i][j]} does not exist in dataset media_dir `{media_dir}`. Use original path."
                            )
